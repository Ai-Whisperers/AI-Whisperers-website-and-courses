/**
 * Color Design Tokens
 * Centralized color system with semantic naming
 * Supports 6 theme variants with 11-shade color scales
 */

// Color scale type (50-950 shades)
export interface ColorScale {
  50: string
  100: string
  200: string
  300: string
  400: string
  500: string
  600: string
  700: string
  800: string
  900: string
  950: string
}

// Theme color structure
export interface ThemeColors {
  id: string
  name: string
  description: string
  primary: ColorScale
  secondary: ColorScale
  accent: ColorScale
  success: string
  warning: string
  error: string
  neutral: ColorScale
}

// All available color themes
export const colorThemes = {
  blueProfessional: {
    name: "Blue Professional",
    id: "blueProfessional",
    description: "Professional and trustworthy blue tones",
    primary: {
      50: "#eff6ff",
      100: "#dbeafe",
      200: "#bfdbfe",
      300: "#93c5fd",
      400: "#60a5fa",
      500: "#3b82f6",
      600: "#2563eb",
      700: "#1d4ed8",
      800: "#1e40af",
      900: "#1e3a8a",
      950: "#172554"
    },
    secondary: {
      50: "#f8fafc",
      100: "#f1f5f9",
      200: "#e2e8f0",
      300: "#cbd5e1",
      400: "#94a3b8",
      500: "#64748b",
      600: "#475569",
      700: "#334155",
      800: "#1e293b",
      900: "#0f172a",
      950: "#020617"
    },
    accent: {
      50: "#f0f9ff",
      100: "#e0f2fe",
      200: "#bae6fd",
      300: "#7dd3fc",
      400: "#38bdf8",
      500: "#0ea5e9",
      600: "#0284c7",
      700: "#0369a1",
      800: "#075985",
      900: "#0c4a6e",
      950: "#082f49"
    },
    success: "#22c55e",
    warning: "#f59e0b",
    error: "#ef4444",
    neutral: {
      50: "#fafafa",
      100: "#f5f5f5",
      200: "#e5e5e5",
      300: "#d4d4d4",
      400: "#a3a3a3",
      500: "#737373",
      600: "#525252",
      700: "#404040",
      800: "#262626",
      900: "#171717",
      950: "#0a0a0a"
    }
  },

  greenNature: {
    name: "Green Nature",
    id: "greenNature",
    description: "Natural and eco-friendly green tones",
    primary: {
      50: "#f0fdf4",
      100: "#dcfce7",
      200: "#bbf7d0",
      300: "#86efac",
      400: "#4ade80",
      500: "#22c55e",
      600: "#16a34a",
      700: "#15803d",
      800: "#166534",
      900: "#14532d",
      950: "#052e16"
    },
    secondary: {
      50: "#f7fee7",
      100: "#ecfccb",
      200: "#d9f99d",
      300: "#bef264",
      400: "#a3e635",
      500: "#84cc16",
      600: "#65a30d",
      700: "#4d7c0f",
      800: "#3f6212",
      900: "#365314",
      950: "#1a2e05"
    },
    accent: {
      50: "#edfdf8",
      100: "#d1fae5",
      200: "#a7f3d0",
      300: "#6ee7b7",
      400: "#34d399",
      500: "#10b981",
      600: "#059669",
      700: "#047857",
      800: "#065f46",
      900: "#064e3b",
      950: "#022c22"
    },
    success: "#22c55e",
    warning: "#f59e0b",
    error: "#ef4444",
    neutral: {
      50: "#fafafa",
      100: "#f5f5f5",
      200: "#e5e5e5",
      300: "#d4d4d4",
      400: "#a3a3a3",
      500: "#737373",
      600: "#525252",
      700: "#404040",
      800: "#262626",
      900: "#171717",
      950: "#0a0a0a"
    }
  },

  purpleCreative: {
    name: "Purple Creative",
    id: "purpleCreative",
    description: "Creative and innovative purple tones",
    primary: {
      50: "#faf5ff",
      100: "#f3e8ff",
      200: "#e9d5ff",
      300: "#d8b4fe",
      400: "#c084fc",
      500: "#a855f7",
      600: "#9333ea",
      700: "#7c3aed",
      800: "#6b21a8",
      900: "#581c87",
      950: "#3b0764"
    },
    secondary: {
      50: "#fdf4ff",
      100: "#fae8ff",
      200: "#f5d0fe",
      300: "#f0abfc",
      400: "#e879f9",
      500: "#d946ef",
      600: "#c026d3",
      700: "#a21caf",
      800: "#86198f",
      900: "#701a75",
      950: "#4a044e"
    },
    accent: {
      50: "#fef7ff",
      100: "#fdebff",
      200: "#fbd5ff",
      300: "#f8b4ff",
      400: "#f382ff",
      500: "#eb49ff",
      600: "#d91cff",
      700: "#b800e6",
      800: "#9500bd",
      900: "#7a0599",
      950: "#520066"
    },
    success: "#22c55e",
    warning: "#f59e0b",
    error: "#ef4444",
    neutral: {
      50: "#fafafa",
      100: "#f5f5f5",
      200: "#e5e5e5",
      300: "#d4d4d4",
      400: "#a3a3a3",
      500: "#737373",
      600: "#525252",
      700: "#404040",
      800: "#262626",
      900: "#171717",
      950: "#0a0a0a"
    }
  },

  orangeSunset: {
    name: "Orange Sunset",
    id: "orangeSunset",
    description: "Warm and energetic orange tones",
    primary: {
      50: "#fff7ed",
      100: "#ffedd5",
      200: "#fed7aa",
      300: "#fdba74",
      400: "#fb923c",
      500: "#f97316",
      600: "#ea580c",
      700: "#c2410c",
      800: "#9a3412",
      900: "#7c2d12",
      950: "#431407"
    },
    secondary: {
      50: "#fefce8",
      100: "#fef9c3",
      200: "#fef08a",
      300: "#fde047",
      400: "#facc15",
      500: "#eab308",
      600: "#ca8a04",
      700: "#a16207",
      800: "#854d0e",
      900: "#713f12",
      950: "#422006"
    },
    accent: {
      50: "#fdf4ff",
      100: "#fae8ff",
      200: "#f5d0fe",
      300: "#f0abfc",
      400: "#e879f9",
      500: "#d946ef",
      600: "#c026d3",
      700: "#a21caf",
      800: "#86198f",
      900: "#701a75",
      950: "#4a044e"
    },
    success: "#22c55e",
    warning: "#f59e0b",
    error: "#ef4444",
    neutral: {
      50: "#fafafa",
      100: "#f5f5f5",
      200: "#e5e5e5",
      300: "#d4d4d4",
      400: "#a3a3a3",
      500: "#737373",
      600: "#525252",
      700: "#404040",
      800: "#262626",
      900: "#171717",
      950: "#0a0a0a"
    }
  },

  tealOcean: {
    name: "Teal Ocean",
    id: "tealOcean",
    description: "Ocean-inspired teal and blue tones",
    primary: {
      50: "#f0fdfa",
      100: "#ccfbf1",
      200: "#99f6e4",
      300: "#5eead4",
      400: "#2dd4bf",
      500: "#14b8a6",
      600: "#0d9488",
      700: "#0f766e",
      800: "#115e59",
      900: "#134e4a",
      950: "#042f2e"
    },
    secondary: {
      50: "#ecfdf5",
      100: "#d1fae5",
      200: "#a7f3d0",
      300: "#6ee7b7",
      400: "#34d399",
      500: "#10b981",
      600: "#059669",
      700: "#047857",
      800: "#065f46",
      900: "#064e3b",
      950: "#022c22"
    },
    accent: {
      50: "#f0f9ff",
      100: "#e0f2fe",
      200: "#bae6fd",
      300: "#7dd3fc",
      400: "#38bdf8",
      500: "#0ea5e9",
      600: "#0284c7",
      700: "#0369a1",
      800: "#075985",
      900: "#0c4a6e",
      950: "#082f49"
    },
    success: "#22c55e",
    warning: "#f59e0b",
    error: "#ef4444",
    neutral: {
      50: "#fafafa",
      100: "#f5f5f5",
      200: "#e5e5e5",
      300: "#d4d4d4",
      400: "#a3a3a3",
      500: "#737373",
      600: "#525252",
      700: "#404040",
      800: "#262626",
      900: "#171717",
      950: "#0a0a0a"
    }
  },

  redEnergy: {
    name: "Red Energy",
    id: "redEnergy",
    description: "Bold and energetic red tones",
    primary: {
      50: "#fef2f2",
      100: "#fee2e2",
      200: "#fecaca",
      300: "#fca5a5",
      400: "#f87171",
      500: "#ef4444",
      600: "#dc2626",
      700: "#b91c1c",
      800: "#991b1b",
      900: "#7f1d1d",
      950: "#450a0a"
    },
    secondary: {
      50: "#fff7ed",
      100: "#ffedd5",
      200: "#fed7aa",
      300: "#fdba74",
      400: "#fb923c",
      500: "#f97316",
      600: "#ea580c",
      700: "#c2410c",
      800: "#9a3412",
      900: "#7c2d12",
      950: "#431407"
    },
    accent: {
      50: "#fefce8",
      100: "#fef9c3",
      200: "#fef08a",
      300: "#fde047",
      400: "#facc15",
      500: "#eab308",
      600: "#ca8a04",
      700: "#a16207",
      800: "#854d0e",
      900: "#713f12",
      950: "#422006"
    },
    success: "#22c55e",
    warning: "#f59e0b",
    error: "#ef4444",
    neutral: {
      50: "#fafafa",
      100: "#f5f5f5",
      200: "#e5e5e5",
      300: "#d4d4d4",
      400: "#a3a3a3",
      500: "#737373",
      600: "#525252",
      700: "#404040",
      800: "#262626",
      900: "#171717",
      950: "#0a0a0a"
    }
  }
} as const

// Theme ID type
export type ThemeId = keyof typeof colorThemes

// Default theme
export const defaultThemeId: ThemeId = 'blueProfessional'

// Semantic color tokens (theme-agnostic semantic naming)
export const semanticColors = {
  // UI Backgrounds
  background: {
    primary: 'var(--color-bg-primary)',
    secondary: 'var(--color-bg-secondary)',
    tertiary: 'var(--color-bg-tertiary)',
    inverse: 'var(--color-bg-inverse)',
    elevated: 'var(--color-bg-elevated)',
    overlay: 'var(--color-bg-overlay)',
  },

  // UI Surfaces
  surface: {
    base: 'var(--color-surface-base)',
    raised: 'var(--color-surface-raised)',
    overlay: 'var(--color-surface-overlay)',
    sunken: 'var(--color-surface-sunken)',
  },

  // Text Colors
  text: {
    primary: 'var(--color-text-primary)',
    secondary: 'var(--color-text-secondary)',
    tertiary: 'var(--color-text-tertiary)',
    inverse: 'var(--color-text-inverse)',
    disabled: 'var(--color-text-disabled)',
    link: 'var(--color-text-link)',
    linkHover: 'var(--color-text-link-hover)',
  },

  // Border Colors
  border: {
    default: 'var(--color-border-default)',
    subtle: 'var(--color-border-subtle)',
    strong: 'var(--color-border-strong)',
    interactive: 'var(--color-border-interactive)',
    focus: 'var(--color-border-focus)',
  },

  // Interactive States
  interactive: {
    default: 'var(--color-interactive-default)',
    hover: 'var(--color-interactive-hover)',
    active: 'var(--color-interactive-active)',
    disabled: 'var(--color-interactive-disabled)',
  },

  // Feedback Colors
  feedback: {
    success: 'var(--color-feedback-success)',
    warning: 'var(--color-feedback-warning)',
    error: 'var(--color-feedback-error)',
    info: 'var(--color-feedback-info)',
  },

  // Glassmorphism
  glass: {
    background: 'var(--color-glass-bg)',
    border: 'var(--color-glass-border)',
    highlight: 'var(--color-glass-highlight)',
  },
} as const

// Helper to get theme by ID
export function getThemeById(themeId: ThemeId): ThemeColors {
  return colorThemes[themeId]
}

// Helper to get all theme IDs
export function getAllThemeIds(): ThemeId[] {
  return Object.keys(colorThemes) as ThemeId[]
}

// Helper to get all themes
export function getAllThemes(): ThemeColors[] {
  return Object.values(colorThemes)
}
